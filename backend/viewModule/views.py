...
def get_chemicals_in_window(request):
    ...
    if state is None:
        return HttpResponseBadRequest()
    y = int(request.GET.get('year', default=latest_year))
    raw = release.objects.filter(geoFilter(request) & filterReleases(request) & Q(
...
def state_total_releases(request):
    ...
    if y != 'None':
        ...
        t_facilitycount = int(release.objects.filter(geoFilter(request) &
...
def all_state_total_releases(request):
    ...
    queryset = release.objects.filter(filters &
                                      geoFilter(request) & Q(year=y)).values('facility__state').annotate(total=Sum('total')).annotate(air=Sum('air')).annotate(water=Sum(
...
def all_county_total_releases(request):
    ...
    queryset = release.objects.filter(filters &
                                      geoFilter(request) & Q(year=y)).values('facility__county',
...
def top_facility_releases(request):
    ...
    release_type = request.GET.get('release_type', default='all').upper()
    if all == 1:
        queryset = release.objects.filter(geoFilter(request) & Q(year=y)).values('facility__name').annotate(total=Sum('on_site')).annotate(land=Sum('land')).annotate(
...
def top_chemicals(request):
    ...
    queryset = release.objects.filter(filters & geoFilter(request) & Q(year=y)).values(
        'chemical__name')
...
def timeline_top_chemicals(request):
    ...
    if state is None:
        return HttpResponseBadRequest()
    chemicals = list(release.objects.filter(filters & geoFilter(request) & filterReleases(request) & Q(year=latest_year)).values_list(
