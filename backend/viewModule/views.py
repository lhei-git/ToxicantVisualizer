...
def points(request):
    ...
    y = int(request.GET.get('year', default=get_current_year()))
...
def XXXfac_count(request):
    ...
    sw_lat = float(request.GET.get('sw_lat', default=0.0))
    sw_lng = float(request.GET.get('sw_lng', default=0.0))
    start = int(request.GET.get('start', default=get_current_year()))
    end = int(request.GET.get('end', default=get_current_year()))
    if state != 'None' and ne_lat==0.0 and sw_lng==0.0 and ne_lng==0.0 and sw_lat==0.0:
...
# gets most recent year from TRI table
def get_current_year():
    current_year = tri.objects.latest('year').year
    return current_year