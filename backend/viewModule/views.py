...
def all_facility_releases(request):
    ...
    if state is None:
        return HttpResponseBadRequest()
    y = int(request.GET.get('year', default=latest_year))
    qs = release.objects.filter(geo_filter(request) & Q(year=y) & Q(total__gt=0)).values('facility__name').annotate(
        total=Sum('total'), air=Sum('air'), water=Sum('water'), land=Sum('land'), off_site=Sum('off_site')).order_by('facility__name')
    return JsonResponse(list(qs), content_type='application/json', safe=False)
